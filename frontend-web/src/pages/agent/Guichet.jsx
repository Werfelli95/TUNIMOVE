...
// Normalisation des noms de villes pour une comparaison flexible sans accents et sans casse (sans accents, minuscules)
...
const Guichet = () => {
    ...
    // Clôture le service de l'agent, envoie le bilan à l'administration et déconnecte
    ...
    // Calcule l'heure réelle d'arrivée du bus à la station de l'agent en ajoutant les durées des étapes
    ...
    // Formate l'heure locale d'arrivée en HH MM pour l'affichage dans les listes déroulantes
    ...
    // Au démarrage, charge les lignes, bus, tarifs et les infos du guichet de l'agent connecté
    useEffect(() => {
                        ...
                        if (guichetRes.ok) {
                            ...
                            if (guichetData) {
                                ...
                                if (filteredLignes.length === 1) {
                                    ...
                                    if (match) setArretDepart(match.arret);
    ...
    // Récupère les tickets vendus par l'agent pour la journée en cours (pour l'historique et la clôture)
    ...
    // Met à jour les données chaque fois que l'agent change de mode Vente, Historique ou Clôture (Vente / Historique / Clôture)
    useEffect(() => {
    ...
    // Met à jour en temps réel la liste des sièges occupés quand le trajet ou l'horaire change
    useEffect(() => {
    ...
    // Sécurité : si l'horaire sélectionné est dépassé de plus de 5 minutes, on le réinitialise automatiquement
    useEffect(() => {
...
// Construit la liste ordonnée et complète des stations de départ, intermédiaires et d'arrivée (départ + intermédiaires + arrivée)
...
// Génère le plan de cabine interactif selon la capacité du bus sélectionné
const renderSeats = () => {
...
// Gère la vente en envoyant les données au serveur puis en déclenchant l'impression du billet
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                if (foundLigne && myGuichet) {
                                    ...
                                    if (match) setArretDepart(match.arret);
                                    else setArretDepart(foundLigne.ville_depart);
                                } else if (foundLigne) {
